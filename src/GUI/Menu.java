...
/**
 * Crea el menú principal asociado a un sistema.
 *
 * @param sistema implementación de {@link logica.SistemaIn} usada por la GUI.
 */
public class Menu {
	...
	/**
	 * Inicia la aplicación gráfica: carga datos del sistema e instancia la ventana principal.
	 * Construye las pestañas principales para Administrador, Coordinador y Estudiante.
	 */
	public void iniciar() {
	...
	private void vistaAdministrador(JTabbedPane tabs) {
		...
		btnCrear.addActionListener(e -> {
			...
			String rol = JOptionPane.showInputDialog(panelAdmin, "Rol (COORDINADOR / ESTUDIANTE):");
			if (rol == null || rol.trim().isEmpty())
				return;

			String extra = JOptionPane.showInputDialog(panelAdmin, "Información adicional (opcional):");
			if (extra == null)
				extra = "";
			...
			if (ok) {
				...
				JOptionPane.showMessageDialog(panelAdmin, "No se pudo crear el usuario.", "Error",
		...
		btnModificar.addActionListener(e -> {
			...
			if (fila == -1) {
				JOptionPane.showMessageDialog(panelAdmin, "Selecciona un usuario en la tabla.", "Atención",
			...
			if (!rolActual.equalsIgnoreCase("ESTUDIANTE") && !rolActual.equalsIgnoreCase("COORDINADOR")) {

				JOptionPane.showMessageDialog(panelAdmin,
						"Solo se pueden modificar cuentas de ESTUDIANTE o COORDINADOR.", "Operación no permitida",
						JOptionPane.WARNING_MESSAGE);
				return;
			}

			String nuevoRol = JOptionPane.showInputDialog(panelAdmin, "Nuevo rol (ESTUDIANTE / COORDINADOR):",
					rolActual);
			...
			if (!nuevoRol.equals("ESTUDIANTE") && !nuevoRol.equals("COORDINADOR")) {
				JOptionPane.showMessageDialog(panelAdmin, "El rol debe ser ESTUDIANTE o COORDINADOR.", "Rol inválido",
						JOptionPane.ERROR_MESSAGE);
				return;
			}

			String nuevoExtra = JOptionPane.showInputDialog(panelAdmin, "Nueva info adicional:", extraActual);
			if (nuevoExtra == null)
				nuevoExtra = "";
			...
			if (usuarioBase == null) {
				JOptionPane.showMessageDialog(panelAdmin, "Usuario no encontrado en el sistema.", "Error",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			Usuario modificado = new Usuario(nombreActual, passFinal, nuevoRol, nuevoExtra.trim());
			...
			if (ok) {
				...
				JOptionPane.showMessageDialog(panelAdmin, "No se pudo modificar el usuario.", "Error",
						JOptionPane.ERROR_MESSAGE);
			}
		});
		...
		btnEliminar.addActionListener(e -> {
			...
			if (rol.equalsIgnoreCase("ADMIN")) {
				JOptionPane.showMessageDialog(panelAdmin, "No se pueden eliminar cuentas de ADMINISTRADOR.",
						"Operación no permitida", JOptionPane.WARNING_MESSAGE);
				return;
			}

			int resp = JOptionPane.showConfirmDialog(panelAdmin, "¿Eliminar usuario '" + nombre + "'?", "Confirmar",
			...
			if (ok) {
				...
				JOptionPane.showMessageDialog(panelAdmin, "No se pudo eliminar el usuario.", "Error",
						JOptionPane.ERROR_MESSAGE);
			}
		});
		...
		btnReset.addActionListener(e -> {
			...
			if (ok) {
				JOptionPane.showMessageDialog(panelAdmin, "Contraseña restablecida a '1234'.");
			} else {
				JOptionPane.showMessageDialog(panelAdmin, "No se pudo restablecer la contraseña.", "Error",
						JOptionPane.ERROR_MESSAGE);
			}
		});
	...
	private void vistaCoordinador(JTabbedPane tabs) {
		...
		btnModificarLinea.addActionListener(e -> {
			...
			if (fila == -1) {
				JOptionPane.showMessageDialog(panelCoord, "Selecciona una certificación.", "Atención",
						JOptionPane.WARNING_MESSAGE);
				return;
			}
			...
			if (c == null) {
				JOptionPane.showMessageDialog(panelCoord, "Certificación no encontrada en el sistema.", "Error",
						JOptionPane.ERROR_MESSAGE);
				return;
			}

			// ---------- Pedimos los NUEVOS valores ----------
			String nuevoNombre = JOptionPane.showInputDialog(panelCoord, "Nuevo nombre:", c.getNombre());
			if (nuevoNombre == null || nuevoNombre.trim().isEmpty())
				return;

			String nuevaDesc = JOptionPane.showInputDialog(panelCoord, "Nueva descripción:", c.getDescripcion());
			if (nuevaDesc == null || nuevaDesc.trim().isEmpty())
				return;

			String txtCred = JOptionPane.showInputDialog(panelCoord, "Créditos necesarios:", c.getCreditosRequeridos());
			if (txtCred == null || txtCred.trim().isEmpty())
				return;

			String txtAnios = JOptionPane.showInputDialog(panelCoord, "Años de validez:", c.getAñosValidez());
			if (txtAnios == null || txtAnios.trim().isEmpty())
				return;
			...
			try {
				...
				Certificacion modificada = new Certificacion(id, nuevoNombre.trim(), nuevaDesc.trim(), nuevosCreditos,
						nuevosAnios);

				boolean ok = sistema.modificarLineaCertificacion(modificada);
				if (ok) {
					JOptionPane.showMessageDialog(panelCoord, "Línea de certificación modificada.");
					refrescarTablaCertificaciones(tablaCert, columnas);
				} else {
					JOptionPane.showMessageDialog(panelCoord, "No se pudo modificar la certificación.", "Error",
							JOptionPane.ERROR_MESSAGE);
				}
			} catch (NumberFormatException exNum) {
				JOptionPane.showMessageDialog(panelCoord, "Créditos y años de validez deben ser números enteros.",
		...
		btnGenerarCertificados.addActionListener(e -> {
			...
			ArrayList<RegistroCertificacion> regs = sistema.getRegistrosCertificacion(idCert);

			if (regs.isEmpty()) {
				mostrarTextoEnDialogo(panelCoord, "Certificados",
			...
			for (RegistroCertificacion r : regs) {
				...
				boolean completado = r.getEstado().equalsIgnoreCase("Completada") || r.getProgreso() >= 100;

				if (!completado)
					continue; // no mostrar los que no han terminado
				...
				sb.append("  Certificación completada en fecha: ").append(r.getFechaRegistro()).append("\n\n");
			}
		...
		btnEstadisticas.addActionListener(e -> {
			...
			for (Certificacion c : lista) {
				ArrayList<RegistroCertificacion> regs = sistema.getRegistrosCertificacion(c.getId());
				...
				sb.append(c.getId()).append(" - ").append(c.getNombre()).append(" | Inscritos: ").append(inscritos)
						.append(" | Progreso promedio: ").append(String.format("%.1f", promedioProg)).append("%\n");
			}
		...
		btnAsignCriticas.addActionListener(e -> {
			...
			if (criticas.isEmpty()) {
				...
				for (Curso c : criticas) {
					sb.append("• ").append(c.getNcr()).append(" - ").append(c.getNombre()).append("\n");
				}
			}
		...
		btnPerfiles.addActionListener(e -> {
			...
			if (ests.isEmpty()) {
				...
				for (Estudiante es : ests) {
					sb.append(es.getRut()).append(" - ").append(es.getNombre()).append(" (").append(es.getCarrera())
							.append(")\n");
				}
			}
		...
		btnValidarAvance.addActionListener(e -> {
			...
			if (regs.isEmpty()) {
				JOptionPane.showMessageDialog(panelCoord, "No hay estudiantes inscritos en esta certificación.",
			...
			for (RegistroCertificacion r : regs) {
				...
				if (r.getProgreso() >= 100 && !r.getEstado().equalsIgnoreCase("COMPLETADA")) {
					r.setEstado("COMPLETADA"); // necesita setEstado(...) en RegistroCertificacion
				}

				sb.append(es.getRut()).append(" - ").append(nombreEst).append(" | Progreso: ").append(r.getProgreso())
	...
	private void vistaEstudiante(JTabbedPane tabs) {
		...
		tablaMalla.setDefaultRenderer(Object.class, new EstadoAsignaturaRenderer(colMalla.length - 1)); // última col =
																										// Estado
		...
		tablaMallaInt.setDefaultRenderer(Object.class, new EstadoAsignaturaRenderer(colMallaInt.length - 1));

		JScrollPane scrollMallaInt = new JScrollPane(tablaMallaInt);
		scrollMallaInt.setBorder(BorderFactory.createTitledBorder("Malla interactiva por semestre"));
		...
		tablaMallaInt.addMouseListener(new java.awt.event.MouseAdapter() {
			...
			public void mouseClicked(java.awt.event.MouseEvent e) {
				if (e.getClickCount() == 2) {
					...
					if (rut.isEmpty()) {
						JOptionPane.showMessageDialog(panelEst, "Ingrese un RUT y cargue los datos primero.",
		...
		scrollMallaGrafica.setBorder(BorderFactory.createTitledBorder("Malla gráfica por semestre"));
		...
		btnCargar.addActionListener(e -> {
			String rut = txtRut.getText().trim();
			if (rut.isEmpty()) {
				JOptionPane.showMessageDialog(panelEst, "Ingrese un RUT.", "Atención", JOptionPane.WARNING_MESSAGE);
				return;
			}

			Estudiante es = sistema.getEstudiante(rut);
			if (es == null) {
				JOptionPane.showMessageDialog(panelEst, "No se encontró estudiante con rut " + rut, "Error",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
		...
		btnPromGeneral.addActionListener(e -> {
			String rut = txtRut.getText().trim();
			if (rut.isEmpty()) {
				JOptionPane.showMessageDialog(panelEst, "Ingrese un RUT y cargue datos primero.", "Atención",
						JOptionPane.WARNING_MESSAGE);
				return;
			}
			double prom = sistema.calcularPromedioGeneral(rut);
			JOptionPane.showMessageDialog(panelEst, "Promedio general del estudiante: " + prom);
		});
		...
		btnPromSemestre.addActionListener(e -> {
			...
			Integer semestreSel = (Integer) JOptionPane.showInputDialog(panelEst, "Seleccione semestre:",
					"Promedio por semestre", JOptionPane.QUESTION_MESSAGE, null, opcionesSem, opcionesSem[0]);

			if (semestreSel == null)
				return;

			double prom = sistema.calcularPromedioSemestre(rut, semestreSel);
			JOptionPane.showMessageDialog(panelEst, "Promedio del semestre " + semestreSel + ": " + prom);
		});
		...
		btnVerInscripciones.addActionListener(e -> {
			String rut = txtRut.getText().trim();
			if (rut.isEmpty()) {
				JOptionPane.showMessageDialog(panelEst, "Ingrese un RUT primero.", "Atención",
						JOptionPane.WARNING_MESSAGE);
				return;
			}
			...
			for (RegistroCertificacion r : regs) {
				...
				sb.append("- ").append(nombreCert).append(" (").append(r.getIdCertificacion()).append(")")
						.append(" | Estado: ").append(r.getEstado()).append(" | Progreso: ").append(r.getProgreso())
						.append("%\n");
			}
			...
			for (RegistroCertificacion r : regs) {
				...
				ArrayList<Curso> faltantes = sistema.getRamosFaltantesCertificacion(rut, r.getIdCertificacion());

				if (faltantes.isEmpty()) {
					sb.append("   - No hay ramos pendientes según los registros.\n\n");
				} else {
					for (Curso cFal : faltantes) {
						sb.append("   - ").append(cFal.getNcr()).append("  ").append(cFal.getNombre()).append("  (Sem ")
								.append(cFal.getSemestre()).append(", ").append(cFal.getCreditos())
								.append(" créditos)\n");
					}
					sb.append("\n");
				}
	...
	private void actualizarMallaParaRut(String rut, javax.swing.table.DefaultTableModel modeloMalla) {
		...
		for (dominio.Curso c : cursos) {
			...
			Object[] fila = { c.getNcr(), c.getNombre(), c.getSemestre(), c.getCreditos(), (nota == null ? "-" : nota),
					estado };
	...
	private void actualizarMallaParaRutYSemestre(String rut, int semestreFiltro,
		...
		for (dominio.Curso c : cursos) {
			...
			if (c.getSemestre() != semestreFiltro)
				continue;
	...
	private void gestionarInscripcionCertificacion(String rut, java.awt.Component parent) {
		...
		if (lista.isEmpty()) {
			javax.swing.JOptionPane.showMessageDialog(parent, "No hay certificaciones disponibles.", "Información",
		...
		String seleccion = (String) javax.swing.JOptionPane.showInputDialog(parent,
				"Selecciona una certificación para inscribirte:", "Inscripción a certificación",
		...
		if (!cumple) {
			javax.swing.JOptionPane.showMessageDialog(parent,
					"No cumples los requisitos académicos para esta certificación.", "Requisitos no cumplidos",
		...
		if (ok) {
			javax.swing.JOptionPane.showMessageDialog(parent, "Inscripción realizada correctamente.", "Éxito",
					javax.swing.JOptionPane.INFORMATION_MESSAGE);
		} else {
			javax.swing.JOptionPane.showMessageDialog(parent, "No se pudo realizar la inscripción.", "Error",
					javax.swing.JOptionPane.ERROR_MESSAGE);
		}
	}

	// Detalle de una asignatura en la malla interactiva
	private void mostrarDetalleAsignaturaInteractiva(String rut, String nrc, java.awt.Component parent) {
		...
		if (cursoSeleccionado == null) {
			JOptionPane.showMessageDialog(parent, "No se encontró la asignatura con NRC " + nrc, "Error",
					JOptionPane.ERROR_MESSAGE);
			return;
		}
		...
		// "Ramos que abre": otros cursos que tienen este NRC en su lista de
		// prerrequisitos
		...
		if (abre.isEmpty()) {
			...
			for (dominio.Curso c : abre) {
				sb.append("- ").append(c.getNcr()).append(" : ").append(c.getNombre()).append(" (sem ")
						.append(c.getSemestre()).append(")\n");
			}
		}
		...
		for (int semestre : mapaSemestres.keySet()) {
			...
			for (Curso c : mapaSemestres.get(semestre)) {
				...
				JLabel lbl = new JLabel(
						"<html><b>" + c.getNombre() + "</b><br/>NRC: " + c.getNcr() + "<br/>" + estado + "</html>");
				cajita.add(lbl);
...
//================== ESTUDIANTE: Inscripción a certificaciones ==================
	...
	private void vistaInscripcionCertificaciones(JTabbedPane subTabs, JTextField txtRut) {
		...
		scrollDetalle.setBorder(BorderFactory.createTitledBorder("Detalle / Requisitos de la certificación"));
		...
		btnVerReq.addActionListener(e -> {
			int fila = tablaCert.getSelectedRow();
			if (fila == -1) {
				JOptionPane.showMessageDialog(panel, "Selecciona una línea de certificación.", "Atención",
						JOptionPane.WARNING_MESSAGE);
				return;
			}
...
//--------- INSCRIBIRSE (con validaciones y prerrequisitos) ----------
		btnInscribir.addActionListener(e -> {
			String rut = txtRut.getText().trim();
			if (rut.isEmpty()) {
				JOptionPane.showMessageDialog(panel, "Ingresa el RUT del estudiante y carga los datos primero.",
						"Atención", JOptionPane.WARNING_MESSAGE);
				return;
			}
			...
			if (yaInscrito) {
				JOptionPane.showMessageDialog(panel,
						"El estudiante ya está inscrito en esta certificación (" + id + ").", "Ya inscrito",
			...
			if (!cumple) {
				...
				if (faltantes.isEmpty()) {
					...
					for (Curso c : faltantes) {
						sb.append("- ").append(c.getNcr()).append("  ").append(c.getNombre()).append("  (Sem ")
								.append(c.getSemestre()).append(", ").append(c.getCreditos()).append(" créditos)\n");
					}
				}
			...
			if (ok) {
				JOptionPane.showMessageDialog(panel, "Inscripción realizada correctamente.");
			} else {
				JOptionPane.showMessageDialog(panel, "No se pudo realizar la inscripción.", "Error",
						JOptionPane.ERROR_MESSAGE);
			}
		});
		...
		btnVerInscritas.addActionListener(e -> {
			String rut = txtRut.getText().trim();
			if (rut.isEmpty()) {
				JOptionPane.showMessageDialog(panel, "Ingresa el RUT del estudiante primero.", "Atención",
						JOptionPane.WARNING_MESSAGE);
				return;
			}

			java.util.ArrayList<RegistroCertificacion> lista = sistema.getCertificacionesInscritas(rut);

			if (lista.isEmpty()) {
				JOptionPane.showMessageDialog(panel, "El estudiante no tiene certificaciones inscritas.", "Información",
						JOptionPane.INFORMATION_MESSAGE);
				return;
			}
			...
			// JOptionPane.showMessageDialog(panel, sp, "Certificaciones inscritas",
			// JOptionPane.INFORMATION_MESSAGE);
